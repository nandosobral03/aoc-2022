...
const part2 = (line) => {
    ...
    for (let i = 0; i < forest.length; i++) {
        for (let j = 0; j < forest[0].length; j++) {
            forest[i][j].visibilityScore = calculateVisibilityScore(forest, i, j);
        }
    }
    const max_visibility_score = forest.map(line => line.reduce((acc, tree) => tree.visibilityScore > acc ? tree.visibilityScore : acc, 0)).reduce((acc2, line) => line > acc2 ? line : acc2, 0);
    console.log("Part 2: ", max_visibility_score);
}
